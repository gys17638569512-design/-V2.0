...
from app.models import BootstrapState, User
...
def test_bootstrap_admin_command_rejects_existing_bootstrap_marker(auth_client) -> None:
    session = get_session_factory()()
    try:
        session.add(BootstrapState(key="initial_admin_created"))
        session.commit()
    finally:
        session.close()
